...
interface Tx { id: string; type: string; amount: string; direction: "credit" | "debit"; description: string | null; created_at: string; order_number?: string; }
    ...
    <div className="max-w-xl mx-auto space-y-6">
      ...
      {user?.referral_code && (
        <div className="bg-[#0d1f35] border border-[#1a3060] rounded-2xl p-4 flex items-center justify-between">
          ...
          <button
            onClick={() => navigator.share?.({ text: `Use my LUI referral code: ${user.referral_code}` })
            ...
            {txs.map((tx) => (
              <Link key={tx.id} href={`/app/transaction/${tx.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {tx.description && (
                    <p className="text-[#8b9ab4] text-xs truncate">{tx.description}</p>
                  )}
